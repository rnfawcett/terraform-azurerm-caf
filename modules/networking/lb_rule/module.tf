...
resource "azurerm_lb_rule" "lb" {
  ...
  probe_id                       = can(var.settings.lb_probe.id) ? var.settings.lb_probe.id : var.remote_objects.lb[try(var.settings.loadbancer.lz_key, var.client_config.landingzone_key)][var.settings.lb_probe.key].id
