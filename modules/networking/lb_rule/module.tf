...
resource "azurerm_lb_rule" "lb" {
  ...
  backend_address_pool_ids       = [can(var.settings.backend_address_pool.id) ? var.settings.backend_address_pool.id : var.remote_objects.lb[try(var.settings.loadbalancer.lz_key, var.client_config.landingzone_key)][var.settings.backend_address_pool.key].id]
  probe_id                       = can(var.settings.lb_probe.id) ? var.settings.lb_probe.id : var.remote_objects.lb[try(var.settings.loadbalancer.lz_key, var.client_config.landingzone_key)][var.settings.lb_probe.key].id
